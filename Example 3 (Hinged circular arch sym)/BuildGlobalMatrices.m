function [ks,L1] = BuildGlobalMatrices(Struct_dof,Elem_num,Ele_Node_num,Node_update,E,A,Iz)
L0 = zeros(Elem_num,1);
L1 = zeros(Elem_num,1);
ks = zeros(Struct_dof,Struct_dof);
for n     = 1:1:Elem_num
    Ni    = Ele_Node_num(n,1);Nj=Ele_Node_num(n,2);
    L0(n) = ElementLength2DB(Node_update(Ni,1),Node_update(Ni,2), ...
        Node_update(Nj,1),Node_update(Nj,2));
    L1(n) = L0(n);
    T     = Transformationmatrix2DB(Node_update(Ni,1),Node_update(Ni,2), ...
        Node_update(Nj,1),Node_update(Nj,2));
    ke    = Elasticmatrix2DB(E,A,L1(n),Iz);
    K     = T'*ke*T;
